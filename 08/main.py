...
def login():
    if request.method == 'GET':
        ...
        flash("Bad username or password... Sorry.")
...
def register():
    if request.method == 'POST':
        ...
        if not is_email(email):
            flash('Zoinks! "' + email + '" does not seem like an email address.')
...
def is_email(string):
    ...
    # There is an embedded language called 'regular expression' that would crunch this
    # implementation down to a one-liner, but we'll keep it simple:
...
def rate_movie():
    ...
    # If we didn't redirect by now, the all is well.
    movie.rating = rating
...
app.secret_key = 'A0Zr90j/3yX R!XHH!jmN]LWX/,?RU'
