...
def login():
    if request.method == 'GET':
        ...
        if not existing_user:
            flash("You don't have an account yet.")
            ...
            flash("Please enter the right password, thanks.")
            return redirect('/login')
...
#               Noticed that we've already created a 'login' link in the upper-right corner of the page that'll connect to it.
...
def register():
    if request.method == 'POST':
        ...
        if not is_email(email):
            flash('Zoinks! "' + email + '" does not seem like an email addres...')
            return redirect('/register')

        # [X] TODO 1 -- Validate that form value of 'verify' matches password.
        if password != verify:
            flash("Oh no. You didn't enter the same password.")
            return redirect('/register')
        # [X] TODO 2 -- Validate that there is no user with that email already.
        ...
        if not existing_user:
            ...
            flash("You already have an account, silly.")
            return redirect('/register')
...
def is_email(string):
    ...
    # There is an embedded language called 'regular expression' that would crunc this implementation down
...
def rate_movie():
    ...
    if movie not in get_watched_movies():
        ...
        # So, we redirect them back to the front page and tell them what went wrong.
...
def add_movie():
    ...
    if new_movie_name in terrible_movies:
        error = "Trust ne, you don't want to add '{0}' to your Watchlist.".format(new_movie_name)
        return redirect("/?error=" + error)
...
# [X] TODO 5 -- Modify this function to rely on a list of endpoints that users can visit    
