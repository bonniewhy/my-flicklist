...
def register():
    if request.method == "POST":
        ...
        if not is_email(email):
            flash('Zoinks!! "' + email + '" does not look like an actual email address. D:')
            return redirect('/register')

        email_db_count = User.query.filter_by(email = email).count()

        if email_db_count > 0:
            flash('Yikes! "' + email + '" is already taken. Shucks.')
            return redirect('/register')
...
def crossoff_movie():
    ...
    if not crossed_off_movie:
        return redirect("/?error=Attempt to watch a movie that is unknown to this database.")
